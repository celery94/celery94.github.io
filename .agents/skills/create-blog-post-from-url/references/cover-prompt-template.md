...
- `final_prompt` 为必填字段，不能留空；在生图 prompt 定稿后回填（可先写完 brief 其余字段再补），校验脚本会拒绝空值
...
每篇文章进入封面阶段时只确定 1 个 `selected_style`：用户明确指定时用用户指定；否则从下表 40 项里随机选 1 项。封面 brief、封面 prompt 和图片验收使用同一个风格方向。
