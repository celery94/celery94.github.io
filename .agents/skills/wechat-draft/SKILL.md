...
5. **随机应用设计灵感**，从内置 `design/` 目录中随机选取一份设计参考文档（Apple / Linear / Mintlify / Notion / Pinterest / Spotify），将其色彩和排版语言映射为公众号兼容的内联样式，叠加到现有样式预设之上
6. **调用新增草稿接口**，将文章保存到草稿箱
7. **向用户报告结果**，包括返回的 `media_id` 和本次随机选用的设计参考名称
...
- **设计灵感覆盖层**：每次生成草稿时，脚本会从内置 `design/` 目录（apple / linear / mintlify / notion / pinterest / spotify）随机选取一份设计参考，提取其色彩与气质信息，生成公众号兼容的 CSS 覆盖层叠加到现有样式预设之上。覆盖层仅调整标题色、链接色、引用块配色、表格/分割线边框等可见元素，不影响内容结构，也不使用 WeChat 不支持的高级 CSS。所选设计名称会在脚本输出和最终 result JSON 中的 `design_reference` 字段中体现
...
3. 将 Markdown 转换为 HTML，并把默认样式与额外 CSS 内联到最终内容
  - “参考链接”章节中的 Markdown 链接会转换成以原始 URL 为 `href` 和可见文本的 `<a>` 标签
...
- 本次随机选用的设计灵感参考：`design_reference` 字段（如 `"Apple"`、`"Linear"` 等）
