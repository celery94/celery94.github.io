...
def load_raw_content(args: argparse.Namespace) -> Tuple[Optional[str], Optional[str]]:
    """从 --content 或 --content-file 加载原始内容。"""
    if args.content_file:
        with open(args.content_file, "r", encoding="utf-8") as handle:
            return handle.read(), os.path.abspath(args.content_file)
    if args.content is not None:
        return args.content, None
    return None, None


def detect_content_format(args: argparse.Namespace, source_path: Optional[str]) -> str:
    """根据参数和文件扩展名判断内容格式。"""
...
def collect_newspic_image_paths(args: argparse.Namespace) -> List[str]:
    ...
    if args.image_dir:
        ...
        if not image_dir.is_dir():
            raise NotADirectoryError(f"图片目录无效: {args.image_dir}")

        for child in sorted(image_dir.iterdir(), key=lambda item: item.name.lower()):
    ...
    if len(image_paths) > MAX_NEWSPIC_IMAGES:
        print(
            "      图片消息最多支持 20 张，"
            f"当前收到 {len(image_paths)} 张，已截断为前 {MAX_NEWSPIC_IMAGES} 张"
        )
        image_paths = image_paths[:MAX_NEWSPIC_IMAGES]
...
def load_content(args: argparse.Namespace) -> Tuple[str, str, str]:
    ...
    raw, source_path = load_raw_content(args)
    if raw is None:
        raise ValueError("必须提供 --content 或 --content-file 之一")

    content_format = detect_content_format(args, source_path)
...
def prepare_newspic_content(args: argparse.Namespace) -> str:
    """准备图片消息正文，优先使用显式内容，否则回退到摘要或标题。"""
    raw_content, source_path = load_raw_content(args)
    if raw_content is None:
        fallback = args.digest or args.title
        return normalize_plain_text(fallback)

    content_format = detect_content_format(args, source_path)
    if content_format == "markdown":
        plain_text = BeautifulSoup(markdown_to_html(raw_content), "html.parser").get_text("\n", strip=True)
    elif content_format == "html":
        plain_text = BeautifulSoup(strip_full_document(raw_content), "html.parser").get_text("\n", strip=True)
    else:
        plain_text = raw_content

    normalized = normalize_plain_text(plain_text)
    return normalized or normalize_plain_text(args.digest or args.title)


def build_news_article(args: argparse.Namespace, access_token: str) -> dict:
    """构建图文草稿请求体。"""
    thumb_media_id = args.thumb_media_id
    if args.cover_image and not thumb_media_id:
        print(f"[2/4] 上传封面图片: {args.cover_image} ...")
        thumb_media_id = upload_cover_image(access_token, args.cover_image)
        if thumb_media_id:
    ...
    print("[3/4] 准备文章内容 ...")
    html_content = prepare_content(args, access_token)
...
def build_newspic_article(args: argparse.Namespace, access_token: str) -> dict:
    """构建图片消息草稿请求体。"""
    print("[2/4] 整理图片消息图片 ...")
    image_paths = collect_newspic_image_paths(args)
    if not image_paths:
        raise ValueError("newspic 模式至少需要一张图片")

    print("[3/4] 上传图片消息图片 ...")
    uploaded_media_ids: List[dict] = []
    seen_uploads: Dict[str, Optional[str]] = {}
    ...
    for image_path in image_paths:
        ...
        if image_path not in seen_uploads:
            print(f"      上传图片消息图片: {image_path}")
            seen_uploads[image_path] = upload_newspic_image(access_token, image_path)

        media_id = seen_uploads[image_path]
        if not media_id:
            raise RuntimeError(f"图片消息图片上传失败: {image_path}")

        uploaded_media_ids.append({"image_media_id": media_id})

    return {
        "article_type": "newspic",
        "title": args.title,
        "content": prepare_newspic_content(args),
        "need_open_comment": 1,
        "only_fans_can_comment": 0,
        "image_info": {
            "image_list": uploaded_media_ids,
        },
    }
...
def validate_args(parser: argparse.ArgumentParser, args: argparse.Namespace) -> None:
    ...
    if args.extra_css_file:
        print("      newspic 模式会忽略 --extra-css-file；图片消息正文只支持纯文本")
...
def main():
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="图片消息图片路径，可重复传入；仅用于 --article-type newspic",
    )
    parser.add_argument(
        "--image-dir",
        default=None,
    ...
    if args.article_type == "news":
        article = build_news_article(args, token)
    else:
        article = build_newspic_article(args, token)
    ...
    result = {
        ...
        "image_count": len(article.get("image_info", {}).get("image_list", [])),
